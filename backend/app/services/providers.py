...
async def test_provider_connectivity(
    ...
    # Test the /models endpoint to verify API connectivity and credentials
    url = construct_api_url(normalize_base_url(provider.base_url), "models")
...
async def test_provider_direct(
    ...
    # Test the /models endpoint to verify API connectivity and credentials
    url = construct_api_url(normalize_base_url(base_url), "models")
    start = time.perf_counter()
    try:
        async with httpx.AsyncClient(timeout=request_timeout, follow_redirects=True) as client:
            response = await client.get(url, headers=headers)
